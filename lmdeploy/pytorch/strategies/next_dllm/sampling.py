...
class NextDLLMSamplingStrategy(ARSamplingStrategy):
    """Sampling strategy for autoregressive models."""

    def __init__(self, pad_token_id: int, next_dllm_block_length: int) -> None:
    ...
    def make_sampling_inputs(self, seqs: SeqList) -> SamplingInputs:
        ...
        for name in update_attr_names:
            ...
            if attr is None:
                continue
            repeats = (next_dllm_block_length, ) + (1, ) * (attr.dim())
        ...
        if len(out.response_formats) > 0:
            ...
            for resp in out.response_formats:
                new_resp_formats += [resp] * next_dllm_block_length
            out.response_formats = tuple(new_resp_formats)

        out.batch_size *= next_dllm_block_length
